...
db_sampler = dict(
    ...
    classes=class_names,
    # trailer/motorcycle/bicycle were 6 (CenterPoint's nuScenes rates). In the
    # 20-epoch run at those rates all three collapsed once pasting stopped:
    # predictions/GT fell 0.38->0.15 (motorcycle), 0.86->0.08 (bicycle),
    # 0.27->0.14 (trailer) between epoch 12 and 16, and their AMOTA stayed ~0,
    # while every class pasted at <=4 grew across the fade -- bus at 4 went from
    # 0.000 in the non-augmented run to 0.154. 4 is the highest rate this model
    # has been shown to recover from.
    sample_groups=dict(
        car=2, truck=3, bus=4, trailer=4,
        motorcycle=4, bicycle=4, pedestrian=2),
    points_loader=dict(
